                    ...
                    <div className='flex items-center gap-6' >
                        {pages.map((i, index) => (
                            <Link className='lg:block hidden' key={index} href={i.link} >
                                <button className={`text-[15px] p-2 font-bold hover:text-gray-600 ${router.pathname === i.link || router.pathname.includes(i.link2) || router.pathname.includes(i.link3) ? "text-stone-900" : "text-zinc-400 text"} cursor-pointer transition-all duration-300`} >
                                    {i.name}
                                </button>
                            </Link>
                        ))
